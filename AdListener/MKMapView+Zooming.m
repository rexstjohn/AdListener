...
- (MKMapRect)translateMapRect:(MKMapRect)oldMapRect
    ...
    // get original map coordinates
    MKCoordinateRegion oldRegion = MKCoordinateRegionForMapRect(oldMapRect);
    
    // get original rectangle
    CGRect oldRect = [self convertRegion:oldRegion toRectToView:nil];
    
    // get the transformed rectangle
    CGFloat xPadding = (oldRect.size.width * leftScreenPoints) / (view.frame.size.width - leftScreenPoints);
    CGFloat yPadding = (oldRect.size.height * topScreenPoints) / (view.frame.size.height - topScreenPoints);
    CGRect transformedRect = CGRectMake(oldRect.origin.x - xPadding,
                                        oldRect.origin.y - yPadding,
                                        oldRect.size.width + xPadding,
                                        oldRect.size.height + yPadding);

    // get transformed region
    MKCoordinateRegion transformedRegion = [self convertRect:transformedRect toRegionFromView:nil];

    // create a map rectangle from step 7
    MKMapRect mapRect = [MKMapView mapRectForRegion:transformedRegion];
